...
int cisco_put_80211_wtp_radio_cfg(uint8_t *dst, int rid, mbag_t radio)
{
...
{
	int l = cisco_put_80211_wtp_radio_cfg(dst+10,rid,radio);
	return l+cw_put_elem_vendor_hdr(dst,CW_VENDOR_ID_CISCO,CW_CISCO_WTP_RADIO_CFG,l);
}

int cisco_out_80211_wtp_radio_cfg(struct conn *conn, struct cw_action_out *a, uint8_t * dst)
{
